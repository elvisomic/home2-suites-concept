const Slika = ({ src, cName }: { src: string, cName: string }) => {
    return (
        <section>
            ...
            <img loading='lazy' src={src} className={`object-cover w-full transform transition-transform duration-400 hover:scale-120 ${cName} h-140`}  alt={`Image showing ${cName}`} />
        </section>
    )
}
...
const Text = ({ t, cName }: { t: string, cName?: string }) => {
    return <h2 className={`absolute text-[28px] text-white/80 z-20 mt-4 ml-4 ${cName}`}>{t}</h2>
}
